export const passwordGenerator = {
  ...
  "passwordGenerator.presetsDesc": "为不同安全级别预配置的密码设置。点击「快速运行」使用预设设置生成：",
  ...
  "passwordGenerator.seo.desc": "强大且唯一的密码对于安全至关重要。该工具使用 <strong className=\"text-foreground\">加密安全</strong> 的随机数生成（通过 Web Crypto API 的 crypto.getRandomValues）来创建几乎无法猜测或通过暴力破解的密码。实现支持随机字符生成和基于助记词的方法。",
  
  "passwordGenerator.tech.title": "技术实现",
  "passwordGenerator.tech.randomTitle": "随机密码生成：",
  "passwordGenerator.tech.randomList1": "使用 crypto.getRandomValues() 获取加密安全的随机性",
  "passwordGenerator.tech.randomList2": "根据用户偏好动态构建字符池",
  "passwordGenerator.tech.randomList3": "可选排除易混淆字符（0、O、1、l、I 以提高可读性）",
  "passwordGenerator.tech.randomList4": "均匀分布确保每个字符位置的可能性相等",
  "passwordGenerator.tech.randomList5": "支持 4 到 64 个字符的长度，包含多种字符类型",
  
  "passwordGenerator.tech.passphraseTitle": "助记词生成：",
  "passwordGenerator.tech.passphraseList1": "从 100 多个常用英语单词中进行字典选择",
  "passwordGenerator.tech.passphraseList2": "每个单词增加约 6.6 位熵",
  "passwordGenerator.tech.passphraseList3": "可选大写和添加数字/符号",
  "passwordGenerator.tech.passphraseList4": "可自定义分隔符（连字符、下划线、空格等）",
  "passwordGenerator.tech.passphraseList5": "单词数范围：3-8 个单词，适用于不同安全级别",

  "passwordGenerator.features.title": "主要功能",
  "passwordGenerator.feature.secure.title": "加密安全",
  "passwordGenerator.feature.secure.desc": "使用 Web Crypto API 获取真正的随机性",
  ...
  "passwordGenerator.bestPractices.boundary1": "✅ 关键 - 防止跨多个服务的凭证填充攻击",
  "passwordGenerator.bestPractices.item2": "密码长度至少 12-16 个字符",
  "passwordGenerator.bestPractices.boundary2": "✅ 推荐 - 高价值账户（银行、邮箱）使用 16 个以上字符",
  ...
  "passwordGenerator.bestPractices.boundary4": "✅ 必要 - 切勿重复使用密码或以不安全的方式记录",
  "passwordGenerator.bestPractices.item5": "在可用时启用双因素认证",
  "passwordGenerator.bestPractices.boundary5": "✅ 强烈推荐 - 添加关键的第二层安全保护",

  "passwordGenerator.faq.title": "常见问题",
  "passwordGenerator.faq.q1": "这个密码生成器安全吗？",
  "passwordGenerator.faq.a1": "是的！我们使用 Web Crypto API（crypto.getRandomValues）提供加密安全的随机数。所有生成都在您的浏览器本地进行。",
  "passwordGenerator.faq.q2": "我的密码会被存储在任何地方吗？",
  "passwordGenerator.faq.a2": "不会。生成的密码仅存在于您的浏览器中，永远不会发送到任何服务器。一旦您关闭页面，除非您保存，否则它们将消失。",
  "passwordGenerator.faq.q3": "什么样的密码才是强密码？",
  "passwordGenerator.faq.a3": "强密码是长（16 个以上字符）、使用多种字符类型、对每个账户唯一、随机的（不基于字典词或个人信息）。",
};
